...
void writeCommand(unsigned char c) {
  ...
  // Set OC pin to 0 to enable SPI receiving
  MAP_GPIOPinWrite(GPIOA3_BASE, 0x10, 0);
  // Set DC pin to 0 to send command
  MAP_GPIOPinWrite(GPIOA1_BASE, 0x10, 0);
  // Transmitting data
  MAP_SPIDataPutNonBlocking(GSPI_BASE, c);
  MAP_SPIDataGet(GSPI_BASE, &b);
  // Set OC pin to 1 to end SPI receiving
...
void Adafruit_Init(void){
  ...
  MAP_GPIOPinWrite(GPIOA2_BASE, 0x2, 0);	  // RESET = RESET_LOW

  for(delay=0; delay<100; delay=delay+1); // delay minimum 100 ns

  MAP_GPIOPinWrite(GPIOA2_BASE, 0x2, 0x2);	  // RESET = RESET_HIGH
