...
void lcdTestChar(void)
{
	...
	while(1){ // loop until all 256 characters are drawn
		...
		drawChar(x, y, num, WHITE, BLACK, 1);
		...
		if(num == 255) //if the font drawn is the last font then break out of the loop and ends the function
			break;
		else
			num++; //set next font to be drawn
	}
	delay(200);
}
